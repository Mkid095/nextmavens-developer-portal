/**
 * GraphQL Service Snapshot Client Cache
 * Snapshot cache management utilities
 */

import type { CachedSnapshot, CacheStats } from './types'
 ...
 * @returns Cache stats
 */
export function getSnapshotCacheStats(): CacheStats {
  return {
 ...
 */
export function cleanupExpiredCacheEntries(formatLog?: (message: string) => string): number {
  const now = Date.now()
  let cleaned = 0
  ...
  if (cleaned > 0 && formatLog) {
    console.log(formatLog(`Cleaned up ${cleaned} expired cache entries`))
  }
