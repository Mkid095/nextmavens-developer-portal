...
export class GraphQLServiceSnapshotClient {
  ...
  async validateGraphQLOperation(projectId: string): Promise<GraphQLOperationResult> {
    ...
    const isEnabled = await this.isGraphQLServiceEnabled(projectId)
    if (!isEnabled) {
 ...
 */
export async function isGraphQLServiceEnabled(projectId: string): Promise<boolean> {
 ...
 * @returns Quota configuration or null
 */
export async function getGraphQLQuotas(
  projectId: string
 ...
 * @returns Rate limit configuration or null
 */
export async function getGraphQLRateLimits(
  projectId: string
...
/**
 * US-007: Helper functions for GraphQL-specific errors
 * These functions create standardized errors for GraphQL operations
 */
